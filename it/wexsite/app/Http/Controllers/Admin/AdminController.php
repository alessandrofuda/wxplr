...
class AdminController extends Controller
{
    public function index(){
		...
		/*	$orders = Order::select(\DB::raw("Count(*)"), 'user_profile.country',
		\DB::raw("Week(orders.created_at)"),\DB::raw("Month(orders.created_at)"))
                            ->join('users','users.id','=','orders.user_id')
                            ->join('user_profile','user_profile.user_id','=','users.id')
                            ->whereIn('item_name', ['Professional Kit', 'Global Tool Box'])
                            ->where('item_type', 'service')
                            ->groupBy('user_profile.country');*/

		$orders = Order::select( \DB::raw('Sum(item_name = "Professional Kit") as professional_kit_count'),
			\DB::raw('Sum(item_name = "Global Toolbox") as global_tool_box_count'),
			'user_profile.country',
			\DB::raw('Week(orders.created_at)'),
			\DB::raw('Month(orders.created_at)')
		//	\DB::raw('GROUP_CONCAT(orders.id)')
			)
			->join('users','users.id','=','orders.user_id')
			->join('user_profile','user_profile.user_id','=','users.id')
			->whereIn('item_name', ['Professional Kit', 'Global Toolbox'])
			->where('item_type', 'service')
			->groupBy('user_profile.country');
		...
		$weekOrders = Order::select( \DB::raw('Sum(item_name = "Professional Kit") as professional_kit_count'),
			\DB::raw('Sum(item_name = "Global Toolbox") as global_tool_box_count'),
			'user_profile.country',
			\DB::raw('Week(orders.created_at)'),
			\DB::raw('Month(orders.created_at)')
		//	\DB::raw('GROUP_CONCAT(orders.id)')
			...
			->where('item_type', 'service')
			->groupBy('user_profile.country')
			->where(\DB::raw('Week(orders.created_at)'),'=', $current_week)
			->get();

		$monthOrders = Order::select( \DB::raw('Sum(item_name = "Professional Kit") as professional_kit_count'),
			\DB::raw('Sum(item_name = "Global Toolbox") as global_tool_box_count'),
			'user_profile.country',
			\DB::raw('Week(orders.created_at)'),
			\DB::raw('Month(orders.created_at)')
		//	\DB::raw('GROUP_CONCAT(orders.id)')
			...
			->where('item_type', 'service')
			->groupBy('user_profile.country')
			->where(\DB::raw('Month(orders.created_at)'),'=',$current_month)
			->get();
